...
CURRENT_VERSION=$(/usr/libexec/PlistBuddy LNPreviewToContextMenu/LNPreviewToContextMenu/Info.plist -c "Print CFBundleShortVersionString")
...
/usr/libexec/PlistBuddy LNPreviewToContextMenu/LNPreviewToContextMenu/Info.plist -c "Set CFBundleShortVersionString $NEXT_VERSION" -c "Set CFBundleVersion 1"
...
gh release create --repo LeoNatan/LNPreviewToContextMenu "$NEXT_VERSION" --title "v$NEXT_VERSION" --notes-file "${RELEASE_NOTES_FILE}"
